...
class OverdueReportTest extends TestCase
{
    ...
    public function test_current_month_report_excludes_today_and_future_due_dates(): void
    {
        [$user, $branch] = $this->createReportUser();

        $oldCustomer = $this->createCustomer($branch, 'Old Debt');
        $yesterdayCustomer = $this->createCustomer($branch, 'Yesterday Debt');
        $todayCustomer = $this->createCustomer($branch, 'Today Debt');
        $futureCustomer = $this->createCustomer($branch, 'Future Debt');

        $this->createInvoice($oldCustomer, '2026-05-01', '2026-05-01', 10000);
        $this->createInvoice($yesterdayCustomer, '2026-06-01', '2026-06-06', 5000);
        $this->createInvoice($todayCustomer, '2026-06-01', '2026-06-07', 7000);
        $this->createInvoice($futureCustomer, '2026-06-01', '2026-06-20', 9000);

        $this->actingAs($user)
            ->get(route('admin.reports.overdue', ['month' => '2026-06']))
            ->assertOk()
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('summary.balance_amount', 15000)
                ->has('overdueInvoices.data', 2)
                ->where('overdueInvoices.data.0.customer.name', 'Old Debt')
                ->where('overdueInvoices.data.0.days_overdue', 37)
                ->where('overdueInvoices.data.1.customer.name', 'Yesterday Debt')
                ->where('overdueInvoices.data.1.days_overdue', 1)
                ->where('overdueAging.0.key', '1_30_days')
                ->where('overdueAging.0.customer_count', 1)
                ->where('overdueAging.1.key', '31_60_days')
                ->where('overdueAging.1.customer_count', 1)
            );
    }

    public function test_overdue_search_updates_rows_totals_and_aging(): void
    {
        [$user, $branch] = $this->createReportUser();

        $matchedCustomer = $this->createCustomer($branch, 'Search Target');
        $otherCustomer = $this->createCustomer($branch, 'Other Customer');

        $this->createInvoice($matchedCustomer, '2026-06-01', '2026-06-01', 12000);
        $this->createInvoice($otherCustomer, '2026-06-01', '2026-06-02', 8000);

        $this->actingAs($user)
            ->get(route('admin.reports.overdue', [
                'month' => '2026-06',
                'q' => 'Search Target',
            ]))
            ->assertOk()
            ->assertInertia(fn (Assert $page) => $page
                ->where('filters.q', 'Search Target')
                ->where('summary.count', 1)
                ->where('summary.customer_count', 1)
                ->where('summary.balance_amount', 12000)
                ->has('overdueInvoices.data', 1)
                ->where('overdueInvoices.data.0.customer.name', 'Search Target')
                ->where('overdueAging.0.customer_count', 1)
            );
    }
